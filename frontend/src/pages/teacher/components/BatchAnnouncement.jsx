...
const BatchAnnouncement = ({ batch }) => {
  ...
  const fetchAnnouncements = async () => {
    try {
      ...
      if (response.success) {
        // Filter announcements to show only batch announcements for this specific batch
        const batchAnnouncements = response.data.filter(
          (announcement) =>
            announcement.type === "batch" &&
            announcement.targetBatch &&
            announcement.targetBatch._id === batch?._id
        );
  ...
  return (
    <Box>
          ...
          >
            <VStack align="start" spacing="1" flex="1" minW="0">
              <HStack spacing="2" wrap="wrap">
                <LuGraduationCap color="#0d9488" size="20" />
                <Text
              ...
              >
                Announcements for {batch?.batchName || "this batch"}
              </Text>
            </VStack>
            <Button
              bg="#0d9488"
                ...
                <VStack spacing="2">
                  ...
                  >
                    {announcements.length === 0
                      ? "Create your first batch announcement to get started"
      ...
      {/* Create Announcement Modal - with batch preset */}
      <CreateAnnouncement
        isOpen={showCreateModal}
        onClose={() => setShowCreateModal(false)}
        onSuccess={handleCreateSuccess}
        presetType="batch"
        presetBatch={batch?._id}
        contextTitle={`Create Announcement for ${batch?.batchName || "Batch"}`}
      />
